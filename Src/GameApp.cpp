...
void GameApp::UpdateScene(float dt)
{
    ...
    if (animateCube)
    {
        phi += 0.3f * dt, theta += 0.37f * dt;XMScalarModAngle(phi);XMScalarModAngle(theta);
       // phi = 
      //  theta = 
    }
    ...
    //m_CBuffer.proj = XMMatrixTranspose(XMMatrixOrthographicLH(10.0f, 10.0f, 1.0f, 1000.0f));//采用正交投影或透视投影
    // 更新常量缓冲区
...
bool GameApp::InitEffect()
{
    ...
    // 创建顶点着色器
    HR(CreateShaderFromFile(nullptr, L"HLSL\\Cube_VS.hlsl", "VS", "vs_5_0", blob.ReleaseAndGetAddressOf()));
    ...
    // 创建像素着色器
    HR(CreateShaderFromFile(nullptr, L"HLSL\\Cube_PS.hlsl", "PS", "ps_5_0", blob.ReleaseAndGetAddressOf()));
...
bool GameApp::InitResource()
{
    ...
    {
        { XMFLOAT3(-1.0f, -1.0f, -1.0f), XMFLOAT4(0.0f, 1.0f, 1.0f, 0.0) },
    ...
    m_CBuffer.view = XMMatrixTranspose(XMMatrixLookAtLH(
        ...
        XMVectorSet(0.0f, 1.0f, 0.0f, 0.0f)//这里可以改摄像机位置
    ));
